...
| Claude Opus 4.7 (via Databricks) | sales 5/5, supply 3/5 | Best result across all backends. Closes 2 supply gaps other models missed. Same 2 irreducible semantic gaps remain. |
| GPT-5.5 Pro (via Databricks) | 0/5, 0/5 | Requires Responses API — not yet supported. See fixme L3. |
| Gemini 2.5 Pro (via Databricks) | 0/5, 0/5 | Catalog crash on full run — likely token expiry, not a Tiri bug. See fixme L4. |
...
6. `DatabricksLLMProvider`: system-only message lists rejected by proxied APIs (Anthropic Claude, Vertex Gemini via Databricks). `_normalize_messages()` now injects a directive placeholder user message. Regression test using `httpx.MockTransport`.
7. Reasoning models (claude-opus-4-x) reject `temperature` parameter at the Databricks proxy. Auto-retry without `temperature` on the specific 400 error string. Preserves determinism on every other endpoint that accepts it. Regression test covers both the retry path AND that other 400 errors still raise.
8. `"Proceed."` placeholder was too vague for Gemini reasoning models — replaced with `"Please respond to the instructions above."` Works across Claude / Gemini / GPT proxies.
